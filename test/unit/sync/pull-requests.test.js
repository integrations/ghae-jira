...
describe('sync/pull-request', () => {
  ...
  describe.each([
    ...
    test('should sync to Jira when Pull Request Nodes have jira references', async () => {
      ...
      nock('https://abc.ghaekube.net').get('/api/v3/repos/integrations/test-repo-name/pulls?per_page=20&page=1&state=all&sort=created&direction=desc')
        .reply(200, pullRequestList);
      nock('https://abc.ghaekube.net').get('/api/v3/repos/integrations/test-repo-name/pulls/51')
        .reply(200, { comments: 0 });
