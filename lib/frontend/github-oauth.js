...
module.exports = function (opts) {
  ...
  async function login(req, res, redirectUrl) {
    ...
    if (!jiraHost && !githubHost) {
      req.log.error('Jira Host or githubHost url is required');
      return res.sendStatus(500);
    }

    if (process.env.GITHUB_INSTANCE === 'ghae') {
      let ghaeInstanceData;
      if (githubHost && jiraHost) {
        const installation = await Installation.getForHost(jiraHost);
        if (installation && !installation.githubHost) {
          Installation.updateGithubHost({ githubHost, jiraHost });
        }
        ghaeInstanceData = await AppSecrets.getForHost(githubHost);
        host = githubHost;
      }
      if (githubHost && !jiraHost) {
        ghaeInstanceData = await AppSecrets.getForHost(githubHost);
      ...
      if (!githubHost && jiraHost) {
        const installation = await Installation.getForHost(jiraHost);
        ghaeInstanceData = (installation && installation.githubHost && await AppSecrets.getForHost(installation.githubHost));
        host = (installation && installation.githubHost);
      }
      if (!ghaeInstanceData) {
        req.log.error(`App Registration is not done for this ${githubHost}`);
        return res.sendStatus(500);
      }
      if (ghaeInstanceData) {
      ...
      const installation = (jiraHost && await Installation.getForHost(jiraHost));
  ...
  async function callback(req, res, next) {
    ...
    if (process.env.GITHUB_INSTANCE === 'ghae') {
      ...
      if (!queryparam.githubHost && (!githubHost || !validateGhaeDomain(githubHost))) {
        return next(new Error(`${githubHost} is not a valid referrer`));
      }
      host = queryparam.githubHost || new URL(githubHost).hostname;
