...
module.exports = function middleware(callback) {
  return withSentry(async (context) => {
    enhanceOctokit(context.octokit, context.log);
    ...
    if (!context.payload.installation && context.payload.action === 'deleted') {
      try {
        await newrelic.startSegment('Middleware: webhook handler', true, async () => callback(context));
      } catch (err) {
        context.sentry.captureException(err);
      }
      return;
    ...
    if (context.payload.sender.type === 'Bot' && !isStateChangeOrDeploymentAction(context.payload.action)) {
      if (!isStateChangeOrDeploymentAction(context.name)) {
        context.log.error({ noop: 'bot', botId: context.payload.sender.id, botLogin: context.payload.sender.login }, 'Halting further execution since the sender is a bot and action is not a state change nor a deployment');
        return;
      }
    }
